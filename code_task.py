...
'''
Rules:
    -any live cell with fewer than 2 live neighbour dies as if by loneliness.
    -any live cell withmore than 3 live neighbors dies,as if by overcrowding.
...
x = y = 55
row = '.'*x+'\n'
with open('empty_board.gol','w') as file:
    print(row*y, file=file)
...
def switch_cell(event):      # turn cell on or off after cliking with mouse cursor
    global alive
    cx = event.x
    cy = event.y
    bx = cx//sz
...
def life(from_startbutton=False, from_stepbutton=False):
    ...
    if from_startbutton is True:     # function has been called from start button and not from recursion
        pauze = False            #  after the pauzebutton  necessary for restart the startbutton is pressed
    checkpauze(pauze)
...
alive = 0
sz = 15                                 # cell size for visualization
steps = 900                              # run a number of steps
                            # milliseconds
step = 1
